...
class GOSNormalize(object):
    ...
    def __init__(self, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225], normalize=None):
        self.mean = mean
        self.std = std
...
class Segmenter:
    def __init__(self, device="cuda", save_path="saved_models"):
        self.init_model(save_path)
        self.transform = transforms.Compose([
            GOSNormalize([0.5, 0.5, 0.5], [1.0, 1.0, 1.0]), self.normalize
        ])


    def load_image(self, im_pil):
        im = np.array(im_pil)
    ...
    def init_model(self, save_path):
        ...
        if not os.path.exists(save_path):
            ...
            os.system(f"rm -r {os.path.join(save_path, 'git')}/xuebinqin/DIS/IS-Net/__pycache__")
            os.system(f"mv {os.path.join(save_path, 'git')}/xuebinqin/DIS/IS-Net/* .")
        ...
        self.hypar["model_path"] = save_path
